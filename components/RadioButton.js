...
const RadioButton = props => {
    ...
    function wrapperCicle(item) {
        const {color, outerWidth, borderWidth, selectValue} = props;
        const {value} = item;
        ...
        return (
            ...
            >
                {initChecked ? innerCircle() : stringified(checked) === (stringified(value)) && innerCircle()}
            </View>
        );
    }
    ...
    function renderLogic() {
        const {data} = props;
        if (data && data.length > 0) {
            if (data[0] && (data[0].value || data[0].value === 0) && data[0].label) {
